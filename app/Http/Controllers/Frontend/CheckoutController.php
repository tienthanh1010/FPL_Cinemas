        ...
        return DB::transaction(function () use ($bookingCode, $provider) {
            ...
            if ($existingPayment) {
                $existingPayment->update([
                    ...
                    'request_payload' => array_merge((array) $existingPayment->request_payload, ['gateway_opened_at' => now()->toIso8601String(), 'retry_reason' => 'MOMO_ATM_RECREATE']),
                    'response_payload' => ['status' => 'INITIATED', 'message' => 'Tạo lại giao dịch MoMo ATM sandbox.'],
                ]);
            ...
            $payment = Payment::create([
                ...
                'request_payload' => [
                    'flow' => $provider === 'MOMO' ? 'MOMO_ATM_SANDBOX' : 'HOSTED_GATEWAY_SIMULATION',
                    'provider' => $provider,
                    'method' => $option['method'],
