...
<style>
  ...
  .seat-inline-legend__chip.swatch-regular { background: linear-gradient(180deg, #e5e7eb 0%, #c7cdd6 100%); }
  .seat-inline-legend__chip.swatch-vip { background: linear-gradient(180deg, #d9dde5 0%, #b8c0cc 100%); }
  .seat-inline-legend__chip.swatch-couple { background: linear-gradient(180deg, #d9dde5 0%, #b8c0cc 100%); }
  .seat-inline-legend__chip.swatch-hold-self { background: linear-gradient(180deg, #7cc7ff 0%, #4ca8f1 100%); }
  .seat-inline-legend__chip.swatch-hold-other { background: linear-gradient(180deg, #74bfff 0%, #5aa8f7 100%); }
  .seat-inline-legend__chip.swatch-booked { background: linear-gradient(180deg, #ff6b57 0%, #f44336 100%); }
  .seat-inline-legend__chip.swatch-blocked { background: linear-gradient(180deg, #f7d84d 0%, #f0b90b 100%); }
  .seat-board-wrap {
    border-radius: 20px;
  ...
  .seat-tile--couple-group {
    min-width: 92px;
    height: 42px;
    padding: 0 .75rem;
    border-radius: 16px;
  }
  .seat-tile--couple-group .seat-tile__code {
    font-size: .74rem;
    letter-spacing: .01em;
  }
  .seat-tile.seat-tile--AVAILABLE { background: linear-gradient(180deg, #e5e7eb 0%, #c7cdd6 100%) !important; color: #475569 !important; }
  .seat-tile.seat-tile--VIP { background: linear-gradient(180deg, #d9dde5 0%, #b8c0cc 100%) !important; color: #475569 !important; }
  .seat-tile.seat-tile--COUPLE { background: linear-gradient(180deg, #d9dde5 0%, #b8c0cc 100%) !important; color: #475569 !important; }
  .seat-tile.seat-tile--HOLD_SELF { background: linear-gradient(180deg, #7cc7ff 0%, #4ca8f1 100%) !important; color: #fff !important; }
  .seat-tile.seat-tile--HOLD_OTHER { background: linear-gradient(180deg, #74bfff 0%, #5aa8f7 100%) !important; color: #fff !important; }
  .seat-tile.seat-tile--RESERVED { background: linear-gradient(180deg, #ff6b57 0%, #f44336 100%) !important; color: #fff !important; }
  .seat-tile.seat-tile--BOOKED { background: linear-gradient(180deg, #ff6b57 0%, #f44336 100%) !important; color: #fff !important; }
  .seat-tile.seat-tile--BLOCKED { background: linear-gradient(180deg, #f7d84d 0%, #f0b90b 100%) !important; color: #714f00 !important; }
  .seat-board-footer {
    display: grid;
...
(() => {
  ...
  const detectSeatSection = (seat) => {
    const seatTypeCode = String(seat?.seat_type_code || '').toUpperCase();
    if (pairSeatCodes.includes(seatTypeCode)) return 'couple';
    if (seatTypeCode === 'VIP') return 'vip';
    return 'regular';
  };
  const buildCoupleSeatGroups = (seats) => {
    const sortedSeats = [...seats].sort((left, right) => Number(left.col_number) - Number(right.col_number));
    const groups = [];
    for (let index = 0; index < sortedSeats.length; index += 2) {
      groups.push(sortedSeats.slice(index, index + 2));
    }
    return groups;
  };
  const buildSeatGroupPresentation = (seatGroup) => {
    const seats = Array.isArray(seatGroup) ? seatGroup.filter(Boolean) : [seatGroup].filter(Boolean);
    const seatIds = seats.map((seat) => Number(seat.id)).filter(Boolean);
    const seatCodes = seats.map((seat) => seat.seat_code).filter(Boolean);
    const available = seats.length > 0 && seats.every((seat) => Boolean(seat.available));
    let seatClass = 'AVAILABLE';
    let stateLabel = seats[0]?.state_label || 'Ghế trống';

    if (seats.every((seat) => seat.state === 'HOLD_SELF')) {
      seatClass = 'HOLD_SELF';
      stateLabel = 'Bạn đang giữ ghế này';
    } else if (seats.some((seat) => seat.state === 'BLOCKED')) {
      seatClass = 'BLOCKED';
      stateLabel = 'Ghế đang khóa / bảo trì';
    } else if (seats.some((seat) => seat.state === 'BOOKED')) {
      seatClass = 'BOOKED';
      stateLabel = 'Ghế đã thanh toán';
    } else if (seats.some((seat) => seat.state === 'RESERVED')) {
      seatClass = 'RESERVED';
      stateLabel = 'Ghế đang chờ thanh toán';
    } else if (seats.some((seat) => seat.state === 'HOLD_OTHER')) {
      seatClass = 'HOLD_OTHER';
      stateLabel = 'Ghế đang có người chọn';
    } else if (seats.some((seat) => pairSeatCodes.includes(String(seat.seat_type_code).toUpperCase()))) {
      seatClass = 'COUPLE';
    } else if (seats.some((seat) => String(seat.seat_type_code).toUpperCase() === 'VIP')) {
      seatClass = 'VIP';
    }
    ...
    return {
      ...
      available,
      stateLabel,
      title: `${seatCodes.join(' · ')} · ${seats[0]?.seat_type_name || 'Ghế'} · ${stateLabel}`,
      codeLabel: seatCodes.join(' · '),
    };
  };
  const groupedSeatSections = () => {
    const sections = { regular: [], vip: [], couple: [] };

    state.seats.forEach((seat) => {
      const sectionKey = detectSeatSection(seat);
      let rowEntry = sections[sectionKey].find((item) => String(item.rowLabel) === String(seat.row_label));
      if (!rowEntry) {
        rowEntry = { rowLabel: String(seat.row_label), seats: [] };
        sections[sectionKey].push(rowEntry);
      }
      rowEntry.seats.push(seat);
    });
      ...
      .map(([sectionKey, rows]) => ({
        ...
        rows: rows
          .map((row) => ({
            ...
            seats: row.seats.sort((left, right) => Number(left.col_number) - Number(right.col_number)),
          }))
          .sort((left, right) => String(left.rowLabel).localeCompare(String(right.rowLabel), undefined, { numeric: true })),
      }))
  ...
  const renderSeatMap = () => {
    ...
    const html = groupedSeatSections().map(({ sectionKey, rows }) => {
      const sectionMeta = sectionConfig[sectionKey] || sectionConfig.regular;
      const rowHtml = rows.map(({ rowLabel, seats }) => {
        const isCoupleSection = sectionKey === 'couple';
        const seatGroups = isCoupleSection ? buildCoupleSeatGroups(seats) : seats;
        const banks = [seatGroups.slice(0, Math.ceil(seatGroups.length / 2)), seatGroups.slice(Math.ceil(seatGroups.length / 2))].filter((bank) => bank.length);

        const bankHtml = banks.map((bank) => `
          <div class="seat-bank ${isCoupleSection ? 'seat-bank--couple' : ''}">
            ${bank.map((seatItem) => {
              const displaySeat = buildSeatGroupPresentation(isCoupleSection ? seatItem : [seatItem]);
              return `
                <button type="button"
                        class="seat-tile seat-tile--${displaySeat.seatClass} ${isCoupleSection ? 'seat-tile--couple-group' : ''}"
                        data-seat-ids="${displaySeat.seatIds.join(',')}"
                        title="${displaySeat.title}"
                        ${displaySeat.available ? '' : 'disabled'}>
                  <span class="seat-tile__code">${displaySeat.codeLabel}</span>
                  <span class="seat-tile__meta">${displaySeat.stateLabel}</span>
                </button>
              `;
            }).join('')}
          </div>
        ...
        return `
          <div class="seat-row">
            ...
            <div class="seat-row__banks ${isCoupleSection ? 'seat-row__banks--couple' : ''}">${bankHtml}</div>
          </div>
        `;
      }).join('');

      return `
        <div class="seat-section seat-section--${sectionKey}">
          <div class="seat-section__head">
            <div>
              <h3 class="seat-section__title">${sectionMeta.title}</h3>
              <p class="seat-section__subtitle">${sectionMeta.subtitle}</p>
            </div>
            <span class="seat-section__badge">${rows.length} dãy</span>
          </div>
          ${rowHtml}
        </div>
      `;
    ...
    seatMap.querySelectorAll('[data-seat-ids]').forEach((button) => {
      const seatIds = String(button.dataset.seatIds || '')
        .split(',')
        .map((value) => Number(value))
        .filter(Boolean);
      button.addEventListener('click', () => toggleSeatSelection(seatIds));
    });
  };
  ...
  const buildSelectedSeatGroups = () => {
    ...
    return state.selectedSeatIds
      ...
      .reduce((groups, seat) => {
        if (processedSeatIds.has(Number(seat.id))) {
          return groups;
        }

        const pairedSeat = findPairSeat(seat);
        const isCoupleSeat = pairSeatCodes.includes(String(seat.seat_type_code).toUpperCase());
        const canGroupPair = isCoupleSeat && pairedSeat && state.selectedSeatIds.includes(Number(pairedSeat.id));
        const seats = canGroupPair ? [seat, pairedSeat].sort((left, right) => Number(left.col_number) - Number(right.col_number)) : [seat];

        seats.forEach((item) => processedSeatIds.add(Number(item.id)));
        groups.push({
          seatIds: seats.map((item) => Number(item.id)),
          seats,
          label: seats.map((item) => item.seat_code).join(' · '),
          isCoupleGroup: seats.length > 1,
        });
        return groups;
      }, []);
  };
  ...
  const renderSelectedSeatAssignments = () => {
    const selectedGroups = buildSelectedSeatGroups();
    const selectedSeats = selectedGroups.flatMap((group) => group.seats);

    selectedSeatCount.textContent = `${selectedSeats.length} ghế`;
    selectedSeatPills.innerHTML = selectedGroups.length
      ? selectedGroups.map((group) => `<span class="selected-seat-pill">${group.label} · ${group.seats[0]?.seat_type_name || 'Ghế'}</span>`).join('')
      : '<span class="booking-empty w-100">Chưa có ghế nào được chọn.</span>';

    selectedSeatEmpty.style.display = selectedGroups.length ? 'none' : 'block';
    selectedSeatAssignments.innerHTML = selectedGroups.map((group) => {
      const ticketSelectors = group.seats.map((seat) => {
        const seatId = Number(seat.id);
        const selectedTicketTypeId = defaultTicketTypeId || 0;
        state.seatTicketTypes[String(seatId)] = selectedTicketTypeId;
        const price = getSeatPrice(seat, selectedTicketTypeId);

        return `
          <div class="mb-3">
            <input type="hidden" data-seat-ticket-type="${seat.id}" value="${selectedTicketTypeId}">
            <div class="seat-selection-card__price">Giá ${seat.seat_code}: <strong>${formatCurrency(price)}</strong></div>
          </div>
        `;
      }).join('');
        ...
        <div class="seat-selection-card">
          <div class="seat-selection-card__head">
            <div>
              ...
              <span>${group.seats[0]?.seat_type_name || 'Ghế'} · ${group.seats.length > 1 ? 'Ghế đôi' : group.seats[0]?.state_label}</span>
            </div>
            <button type="button" class="btn btn-sm btn-cinema-secondary" data-remove-seat-group="${group.seatIds.join(',')}">Bỏ</button>
          </div>
          ${ticketSelectors}
        </div>
      `;
    ...
    selectedSeatAssignments.querySelectorAll('[data-remove-seat-group]').forEach((button) => {
      const seatIds = String(button.dataset.removeSeatGroup || '')
        .split(',')
        .map((value) => Number(value))
        .filter(Boolean);
      button.addEventListener('click', () => toggleSeatSelection(seatIds));
  ...
  const renderSummary = () => {
    ...
    const selectedTicketBreakdown = selectedSeats.map((seat) => {
      ...
      return `${seat.seat_code} (${seat.seat_type_name || 'Ghế'})`;
    }).join(', ');
    ...
    summaryBreakdown.innerHTML = `
      ...
      <div class="summary-breakdown__row"><span>Ghế</span><strong>${selectedTicketBreakdown || 'Chưa chọn'}</strong></div>
  ...
  const toggleSeatSelection = (seatIds) => {
    const resolvedSeatIds = (Array.isArray(seatIds) ? seatIds : [seatIds])
      .map((value) => Number(value))
      .filter(Boolean);
    if (!resolvedSeatIds.length) return;

    const seats = resolvedSeatIds.map((seatId) => getSeatById(seatId)).filter(Boolean);
    if (seats.length !== resolvedSeatIds.length || seats.some((seat) => !seat.available)) {
      return;
    }

    let candidateSeatIds = [...state.selectedSeatIds];
    const alreadySelected = resolvedSeatIds.every((seatId) => candidateSeatIds.includes(seatId));

    if (alreadySelected) {
      candidateSeatIds = candidateSeatIds.filter((value) => !resolvedSeatIds.includes(value));
    } else {
      const nextSeatCount = Array.from(new Set([...candidateSeatIds, ...resolvedSeatIds])).length;
      if (nextSeatCount > maxSeats) {
        showAlert(`Bạn chỉ có thể chọn tối đa ${maxSeats} ghế trong một booking.`, 'error', 'seat-limit');
        return;
      }

      candidateSeatIds.push(...resolvedSeatIds);
    }
