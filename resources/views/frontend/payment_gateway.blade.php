...
<section class="section-space pt-4 pt-lg-5">
  <div class="container-fluid app-container">
    <div class="glass-panel p-4 p-lg-5">
      ...
      <p class="section-copy mb-4">Bấm sang cổng MoMo để nhập thẻ ATM test. Khi MoMo trả kết quả thành công, hệ thống sẽ tự xác nhận vé, chuyển ghế sang đã bán và gửi vé mềm về email người dùng.</p>

      @if(!empty($gatewayError))
        <div class="alert alert-danger rounded-4 mb-4">
          <strong>Không tạo được trang thanh toán MoMo.</strong>
        ...
        <div class="gateway-card">
          @if($isMomo)
            <div class="d-flex justify-content-between align-items-center gap-3 mb-3 flex-wrap">
              <h2 class="h4 mb-0">Thanh toán thẻ ATM qua MoMo</h2>
              <span class="status-pill"><i class="bi bi-credit-card"></i> {{ empty($gatewayError) ? 'Đang chờ nhập thẻ' : 'Chưa tạo được trang thanh toán' }}</span>
            </div>

            <div class="gateway-note mb-3">
              Hệ thống dùng môi trường MoMo Test/Sandbox và chuyển bạn sang trang MoMo để nhập thẻ ATM test. Không cần ngrok khi test cùng máy local, vì sau khi hoàn tất MoMo sẽ chuyển trình duyệt về lại <strong>http://127.0.0.1:8000</strong>.
            </div>

            <div class="gateway-note mb-3">
              <strong>Thẻ ATM test thành công nên dùng:</strong><br>
              Số thẻ: <code>9704 0000 0000 0018</code><br>
              Tên chủ thẻ: <code>NGUYEN VAN A</code><br>
              Ngày phát hành: <code>03/07</code><br>
              Số điện thoại: nhập số bất kỳ hợp lệ, ví dụ <code>0393312307</code>.
            </div>

            @if($payUrl)
              <a class="btn btn-cinema-primary w-100 mb-3" href="{{ $payUrl }}">
                <i class="bi bi-box-arrow-up-right me-2"></i>Sang MoMo nhập thẻ ATM
              </a>
            @else
              <div class="alert alert-warning rounded-4 mb-3">MoMo chưa trả về trang thanh toán. Vui lòng bấm tạo lại giao dịch.</div>
            @endif

            <div class="d-flex flex-wrap gap-2">
              <a class="btn btn-cinema-secondary" href="{{ route('booking.payment', ['booking_code' => $booking->booking_code]) }}"><i class="bi bi-arrow-clockwise me-2"></i>Tạo lại giao dịch MoMo</a>
