...
@section('content')
<<<<<<< HEAD
<section class="page-header"><div><p class="eyebrow">Customer management</p><h2>Khách hàng</h2><p>Tra cứu thành viên, booking, điểm thưởng và tài khoản đăng nhập người dùng trên cùng một màn hình.</p></div><div class="d-flex gap-2"><a href="{{ route('admin.customers.create') }}" class="btn btn-primary"><i class="bi bi-plus-circle me-1"></i>Thêm khách hàng</a></div></section>
=======
...
<<<<<<< HEAD
<div class="card toolbar-card"><div class="card-body"><form class="row g-3 align-items-end" method="GET"><div class="col-lg-8"><label class="form-label">Tìm kiếm</label><input class="form-control" name="q" value="{{ $q }}" placeholder="Tên, số điện thoại, email, email đăng nhập, mã booking..."></div><div class="col-lg-2"><button class="btn btn-primary w-100">Tìm</button></div><div class="col-lg-2"><a href="{{ route('admin.customers.index') }}" class="btn btn-light-soft w-100">Xoá lọc</a></div></form></div></div>
<div class="card"><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Khách hàng</th><th>Tài khoản thành viên</th><th>Booking</th><th>Điểm / Hạng</th><th>Trạng thái</th><th class="text-end">Thao tác</th></tr></thead><tbody>
@forelse($customers as $customer)
<tr>
    <td>
        <div class="list-primary">{{ $customer->full_name }}</div>
        <div class="list-secondary">{{ optional($customer->dob)->format('d/m/Y') ?: 'Chưa có ngày sinh' }} · {{ $customer->gender ?: 'Chưa rõ' }}</div>
        <div class="list-secondary">{{ $customer->phone ?: 'Chưa có SĐT' }} · {{ $customer->email ?: 'Chưa có email' }}</div>
        <div class="list-secondary">{{ $customer->city ?: 'Chưa có thành phố' }}</div>
    </td>
    <td>
        @if($customer->user)
            <div class="list-primary">{{ $customer->user->name }}</div>
            <div class="list-secondary">{{ $customer->user->email }}</div>
            <div class="list-secondary">Đã liên kết tài khoản đăng nhập</div>
        @else
            <div class="list-primary">Chưa liên kết</div>
            <div class="list-secondary">Khách lẻ / dữ liệu nhập tay</div>
        @endif
    </td>
    <td>
        <div class="list-primary">{{ $customer->bookings_count }} đơn</div>
        <div class="list-secondary">{{ $customer->last_booking_code ?: 'Chưa có booking' }}</div>
    </td>
    <td>
        <div class="list-primary">{{ number_format($customer->loyaltyAccount?->points_balance ?? 0) }} điểm</div>
        <div class="list-secondary">Lifetime {{ number_format($customer->loyaltyAccount?->lifetime_points ?? 0) }}</div>
        <div class="list-secondary">Hạng {{ $customer->loyaltyAccount?->tier?->name ?? 'Member' }}</div>
    </td>
    <td><span class="badge {{ ($customer->account_status ?? 'ACTIVE') === 'ACTIVE' ? 'badge-soft-success' : 'badge-soft-warning' }}">{{ $customer->account_status ?? 'ACTIVE' }}</span></td>
    <td class="text-end"><div class="d-inline-flex gap-2"><a href="{{ route('admin.customers.show',$customer) }}" class="btn btn-sm btn-outline-secondary">Xem</a><a href="{{ route('admin.customers.edit',$customer) }}" class="btn btn-sm btn-outline-primary">Sửa</a><form method="POST" action="{{ route('admin.customers.destroy',$customer) }}" onsubmit="return confirm('Xoá khách hàng này?')">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger">Xoá</button></form></div></td>
</tr>
=======
