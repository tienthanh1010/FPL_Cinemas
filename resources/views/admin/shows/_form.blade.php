    ...
    <div class="row g-3 align-items-stretch">
        ...
        <div class="col-lg-4">
            <div class="soft-card h-100 p-3">
                ...
                <div class="small text-muted" id="schedule_preview">Chọn phim, phòng, ngày và giờ bắt đầu để xem lịch.</div>
                <div class="show-conflict-alert" id="schedule_conflict_alert">Có suất chiếu bị trùng giờ trong phòng đã chọn. Các dòng trùng đã được bôi đỏ.</div>
            </div>
        </div>
    </div>
</div>
...
<div class="section-card">
    ...
    <p class="section-description">Chọn phòng và ngày chiếu để xem các khung giờ đã có. Nếu lịch mới bị trùng, hệ thống sẽ bôi đỏ để bạn tránh hoặc sửa trước khi lưu.</p>
...
<script>
    (function () {
        ...
        const roomShowsForSelectedDate = () => {
            const roomId = Number(auditoriumSelect?.value || 0);
            const date = showDate?.value || '';
            if (!roomId || !date) return [];

            return existingShows
                .filter((show) => Number(show.auditorium_id) === roomId && show.date === date)
                .sort((a, b) => String(a.start_clock).localeCompare(String(b.start_clock)));
        };
        ...
        const plannedSlots = () => {
            ...
            if (!duration || startMinutes === null) return [];
            ...
            for (let i = 0; i < count; i++) {
                ...
                slots.push({index: i + 1, start: cursor, end: slotEnd});
                cursor = slotEnd + gap;
            }
        ...
        const renderRoomSchedule = () => {
            ...
            roomScheduleTitle.textContent = roomId && date
                ? `Lịch ${roomText} ngày ${date}`
                : 'Lịch phòng theo ngày';

            if (!roomId || !date) {
                roomScheduleList.innerHTML = '<div class="room-schedule-empty">Chọn phòng chiếu và ngày chiếu để xem lịch hiện có.</div>';
                return;
            }

            const roomShows = roomShowsForSelectedDate();
            const slots = plannedSlots();

            if (!roomShows.length) {
                roomScheduleList.innerHTML = '<div class="room-schedule-empty">Ngày này phòng chưa có suất chiếu nào.</div>';
                return;
            }

            roomScheduleList.innerHTML = roomShows.map((show) => {
                const showStart = toMinutes(show.start_clock);
                const showEnd = toMinutes(show.end_clock);
                const hasConflict = slots.some((slot) => showStart !== null && showEnd !== null && overlaps(slot.start, slot.end, showStart, showEnd));
        ...
        const updateEndTime = () => {
            ...
            if (!duration || startMinutes === null) {
                ...
                if (schedulePreview) schedulePreview.textContent = 'Chọn phim, phiên bản, phòng, ngày và giờ bắt đầu để xem lịch.';
            ...
            if (schedulePreview) {
                schedulePreview.innerHTML = parts.join('');
            }
