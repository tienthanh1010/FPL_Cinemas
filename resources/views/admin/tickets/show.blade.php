    ...
    <div class="col-lg-4">
        <div class="card h-100">
            ...
            <div class="card-body text-center">
                ...
                @if($scanPayload)
                    <div class="mb-3">
                        <img src="{{ ticket_qr_image_url($scanPayload, 260) }}" alt="QR {{ $ticket->ticket_code }}" class="img-fluid rounded-4 border bg-white p-2" style="max-width: 260px;">
                    </div>
                    <div class="small text-secondary mb-2">Có thể quét QR hoặc barcode từ vé cứng / màn hình điện thoại.</div>
                    <div class="mb-3">
                        <img src="{{ ticket_barcode_image_url($ticket->ticket_code, 64) }}" alt="Barcode {{ $ticket->ticket_code }}" class="img-fluid rounded-3 border bg-white p-2">
                    </div>
                    <div class="small text-break">{{ $scanPayload }}</div>
                @else
                    <div class="text-muted">Vé này chưa có dữ liệu mã quét.</div>
                @endif
            </div>
        </div>
    </div>
