<div class="card">
    <div class="card-body row g-3">
        ...
        <div class="col-md-8">
            ...
            <textarea class="form-control" rows="3" name="attributes_text" placeholder="Ví dụ: 1 bắp lớn + 2 nước ngọt">{{ old('attributes_text', $product->attributes['description'] ?? '') }}</textarea>
        </div>
        <div class="col-md-4">
            <label class="form-label">Ảnh sản phẩm / combo</label>
            <input type="url" class="form-control" name="image_url" value="{{ old('image_url', $product->attributes['image_url'] ?? '') }}" placeholder="https://example.com/combo.jpg">
            <div class="form-text">Dán URL ảnh để hiển thị ở trang đặt vé và admin.</div>
            @if(old('image_url', $product->attributes['image_url'] ?? null))
                <div class="mt-2">
                    <img src="{{ old('image_url', $product->attributes['image_url'] ?? '') }}" alt="{{ old('name', $product->name ?: 'Sản phẩm F&B') }}" class="img-fluid rounded-3 border" style="max-height: 180px; object-fit: cover;">
                </div>
            @endif
        </div>
        <div class="col-md-4">
